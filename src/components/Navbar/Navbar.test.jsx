...
describe('NavBar component', () => {
  ...
  xit('renders a banner with the NavBar content', () => {
    const banner = screen.getByRole('banner');
    expect(banner).toBeInTheDocument();
  });

  xit('renders a heading with the name "instagit"', () => {
    const title = screen.getByRole('heading', {name: /instagit/i});
    expect(title).toBeInTheDocument();
  });

  xit('renders a link with the name "home"', () => {
    const title = screen.getByRole('link', {name: /home/i});
    expect(title).toBeInTheDocument();
  });

  xit('renders a link with the name "search"', () => {
    const title = screen.getByRole('link', {name: /search/i});
    expect(title).toBeInTheDocument();
  });

  xit('renders a link with the name "profile"', () => {
